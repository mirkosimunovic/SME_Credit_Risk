"""Causal validation v2 of contract-term effects on SBA default.

Loads data/processed/X_train_v2.csv and y_train_v2.csv.
"""
...
def load_train() -> pd.DataFrame:
    x_path = PROCESSED_DIR / "X_train_v2.csv"
    y_path = PROCESSED_DIR / "y_train_v2.csv"
    if not x_path.exists() or not y_path.exists():
        raise FileNotFoundError("Run scripts/preprocess_v2.py first (X_train_v2 / y_train_v2).")
...
def resolve_confounders(columns: list[str]) -> list[str]:
    ...
    if missing:
        raise KeyError(f"Required confounders missing from X_train_v2: {missing}")
    print(f"  Observed confounders used: {present}")
    return present


def analysis_frame(df: pd.DataFrame, cols: list[str]) -> pd.DataFrame:
    """Numeric subset; drop incomplete rows (no KNN — that belongs in trainer_v2.py)."""
...
def main() -> int:
    ...
    hist_path = FIGURES_DIR / "gcm_falsify_histogram_v2.png"
    print(
    ...
    out_path = RESULTS_DIR / "causal_stability_report_v2.json"
