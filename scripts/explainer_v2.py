...
# Populated from outputs/results/best_hyperparams_v2.json (trainer_v2 final XGBoost).
XGB_BASELINE: dict = {
    "n_estimators": 300,
    "max_depth": 6,
    "learning_rate": 0.05,
...
# ===========================================================================
# trainer_v2.py dumped a StackedFastKNNImputer. Pickle restores attributes
...
class StackedFastKNNImputer:
    ...
    def fit_reference(self, X_fit_enc, encodings):
        # Training already happened in trainer_v2.py. The pickled object carries
...
def load_tuned_xgb_baseline() -> dict:
    """Replace XGB_BASELINE with trainer_v2's final RandomizedSearchCV champion params."""
    hp_path = RESULTS_DIR / "best_hyperparams_v2.json"
    if not hp_path.exists():
        raise FileNotFoundError(
            "Missing outputs/results/best_hyperparams_v2.json. Run scripts/trainer_v2.py first."
        )
    with open(hp_path, encoding="utf-8") as fh:
        payload = json.load(fh)
    params = payload.get("params", {}).get("XGBoost")
    if not isinstance(params, dict) or not params:
        raise KeyError("best_hyperparams_v2.json has no params.XGBoost block.")
...
def main() -> int:
    ...
    model_path = ARTIFACTS_DIR / "xgboost_best_v2.json"
    ...
    beeswarm_path = FIGURES_DIR / "shap_summary_v2.png"
    ...
    bar_path = FIGURES_DIR / "shap_importance_v2.png"
    ...
    print("  Hyperparameters match trainer_v2 tuned XGBoost; only random_state changes.")
    ...
    out_json = RESULTS_DIR / "stability_report_v2.json"
