"""Two-step SME credit-risk trainer v2 (JRFM-oriented).

Uses chronological splits from scripts/preprocess_v2.py:

    data/processed/X_train_v2.csv, y_train_v2.csv
    data/processed/X_oot_v2.csv,   y_oot_v2.csv
...
STEP 1 — Stratified 5-fold CV on the training partition only.
          ...
          Metrics: Recall, AUC-ROC, AUC-PR, F1, Lift@10%. Nested RandomizedSearchCV
          and VIF diagnostics run after each fold's impute/scale.
...
STEP 2 — Impute 100% of X_train, persist that complete matrix as the imputer
          ...
          tuned champions on the imputed/scaled train set; serialize native artifacts.

STEP 3 — Score the chronological OOT holdout with those frozen artifacts,
          bootstrap 95% CIs for AUC-ROC and AUC-PR (n=1000), write ROC plot.
...
def xgb_fixed(spw: float, seed: int = RANDOM_STATE) -> dict:
    return dict(
...
def lgbm_fixed(spw: float, seed: int = RANDOM_STATE) -> dict:
    return dict(
        scale_pos_weight=spw,
        device=LGBM_DEVICE,
        n_jobs=-1,
...
def cat_fixed(spw: float, seed: int = RANDOM_STATE) -> dict:
    return dict(
        scale_pos_weight=spw,
        task_type=CAT_TASK,
        verbose=False,
...
def load_xy(split: str) -> tuple[pd.DataFrame, pd.Series]:
    ...
    x_path = PROCESSED_DIR / f"X_{split}_v2.csv"
    y_path = PROCESSED_DIR / f"y_{split}_v2.csv"
    if not x_path.exists() or not y_path.exists():
        raise FileNotFoundError(
            f"Missing {split} v2 split. Run scripts/preprocess_v2.py first.\n"
            f"  expected {x_path}\n  expected {y_path}"
        )
...
def tune_model(name: str, X: pd.DataFrame, y: pd.Series, spw: float) -> dict:
    """Lightweight RandomizedSearchCV on a stratified subsample (GPU if available)."""
    Xs, ys = _tune_subsample(X, y)
    print(
        f"    Tuning {name} (n_iter={N_TUNE_ITER}, cv={TUNE_CV}, "
        f"subsample={len(Xs):,}, gpu={USE_GPU}) ...",
        flush=True,
    )
    if name == "XGBoost":
        est = XGBClassifier(**xgb_fixed(spw))
        grid = XGB_SEARCH
    elif name == "LightGBM":
        est = LGBMClassifier(**lgbm_fixed(spw))
        grid = LGBM_SEARCH
    else:
        est = CatBoostClassifier(**cat_fixed(spw))
        grid = CAT_SEARCH
    ...
    search.fit(Xs, ys)
    print(f"    {name} best AUC={search.best_score_:.4f}  params={search.best_params_}")
    return dict(search.best_params_)


def instantiate_tuned(name: str, spw: float, params: dict, seed: int = RANDOM_STATE):
    if name == "XGBoost":
        return XGBClassifier(**xgb_fixed(spw, seed), **params)
    if name == "LightGBM":
        return LGBMClassifier(**lgbm_fixed(spw, seed), **params)
    return CatBoostClassifier(**cat_fixed(spw, seed), **params)
...
def run_cross_validation(X: pd.DataFrame, y: pd.Series) -> pd.DataFrame:
    ...
    for fold, (tr_idx, va_idx) in enumerate(cv.split(X, y), start=1):
        ...
        for name in MODEL_ORDER:
            ...
            best = tune_model(name, X_tr_p, y_tr, spw)
            model = instantiate_tuned(name, spw, best)
    ...
    out_path = RESULTS_DIR / "metrics_benchmark_v2.csv"
...
def run_final_fit(
    ...
    imputer_path = ARTIFACTS_DIR / "imputer_v2.joblib"
    scaler_path = ARTIFACTS_DIR / "scaler_v2.joblib"
    ...
    save_paths = {
        "XGBoost": ARTIFACTS_DIR / "xgboost_best_v2.json",
        "CatBoost": ARTIFACTS_DIR / "catboost_best_v2.bin",
        "LightGBM": ARTIFACTS_DIR / "lightgbm_best_v2.txt",
    }
    fitted = {}
    for name in MODEL_ORDER:
        print(f"  Tuning + fitting final {name} on {len(X_train_p):,} training rows ...", flush=True)
        best = tune_model(name, X_train_p, y_train, spw)
    ...
    hp_path = RESULTS_DIR / "best_hyperparams_v2.json"
    with open(hp_path, "w", encoding="utf-8") as fh:
        json.dump({"gpu": USE_GPU, "params": best_params}, fh, indent=2)
...
def run_oot_evaluation(models: dict, X_oot_p: pd.DataFrame, y_oot: pd.Series) -> pd.DataFrame:
    ...
    bench_path = RESULTS_DIR / "metrics_benchmark_v2.csv"
    ...
    out_path = FIGURES_DIR / "roc_curves_comparison_v2.png"
